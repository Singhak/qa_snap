...
const modeInstructions: Record<GenerateTestCasesRequestInput["generationMode"], string> = {
  ...
  REGRESSION: "Provide broad practical coverage across normal, negative, and integration scenarios.",
    ...
    textAttachments.length
      ? [
          ...
          ...textAttachments.map((attachment, index) =>
            [`Document ${index + 1}: ${attachment.name} (${attachment.mimeType})`, truncateText(attachment.textContent ?? "", 5000)].join(
              "\n",
            ),
          ),
        ].join("\n\n")
